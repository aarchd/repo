...
<body class="bg-gray-900 text-blue-100 font-sans p-4 sm:p-8">
  <header class="flex flex-col sm:flex-row justify-center items-center gap-4 mb-6 sm:mb-8">
    ...
    <h1 class="text-2xl sm:text-3xl text-blue-400 font-semibold text-center">Packages</h1>
  </header>
  <div class="overflow-x-auto">
    <table class="min-w-full text-left bg-gray-800 rounded-lg shadow-lg">
      <thead class="bg-blue-600 text-blue-100 font-bold text-sm sm:text-base">
        <tr>
...
for pkg in "$PKG_DIR"/*.pkg.tar.zst; do
  ...
  cat <<EOF >> "$OUTPUT"
        <tr class="even:bg-gray-700 hover:bg-gray-600 text-sm sm:text-base">
          <td class="px-4 sm:px-8 py-2 sm:py-4"><a class="text-blue-400 hover:underline" href="${BASE_URL}/${base}.pkg.tar.zst">${name}</a></td>
